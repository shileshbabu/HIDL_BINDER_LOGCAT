...
Return<int32_t> Simphw::simpfn(int32_t valueIn) {
  ...
  printf("____________________method_calling______________________");
  //TODO passing this messages to framework client
  printf("buff  === %s\n", buff);
...
static void processBuffer(android_logcat_context_internal* context,
    ...
    if ((err < 0) && !context->debug) return;
        if(strstr(entry.message, "newtestapp") != NULL) {
          printf("debug messages:: %s\n",  entry.message);
          strcpy(buff,entry.message);
          //TODO pass this string to binder inst
          }else{
            //printf("false\n");
          }
...
int android_logcat_run_command_thread(android_logcat_context ctx, int argc, char* const* argv, char* const* envp) {
    android_logcat_context_internal* context = ctx;
